...
from data import load_data_new
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inf_graph", type=int, default=1, 
        help="Which inf graph to select. Start counting at 1"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--negative_sampling", type=str, default="node_full", choices=["relation", "node", "relation_full", "node_full"], 
    ...
    if do_test and do_train:
        raise ValueError("Both --do_train and --do_test set! Can only set one!!!")
    ...
    if do_train:
        train_train_data, train_valid_data = load_data_new(dataset_name, data_folder, mode="train")
    ...
    if do_test:
        inf_observe_data, inf_test_data = load_data_new(dataset_name, data_folder, mode="test", inf_graph=args.inf_graph)
        ...
        # print("\n")
        # print("--->", len(inf_msg_edges), len(inf_test_edge), inf_observe_data['num_node'], inf_observe_data['num_relation'])
        # print("\n")
        # exit()
